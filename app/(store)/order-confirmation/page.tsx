  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8 flex flex-col gap-6">
      <div className="bg-[var(--bg-surface)] rounded-xl border border-[var(--border)] p-6 flex flex-col items-center gap-3">
        ...
        <span className="text-sm font-bold text-accent-primary">#TCG-2026-001</span>
      </div>
      ...
      <div className="bg-[var(--bg-surface)] rounded-xl border border-[var(--border)] p-4 flex flex-col gap-3">
        ...
        {items.map((item, i) => (
          <div key={i} className="flex justify-between text-sm">
            <span className="text-text-secondary">{item.name} x{item.qty}</span>
            <span className="text-text-primary">{formatPrice(item.price * item.qty)}</span>
          </div>
        ))}
        <Divider />
        ...
        <div className="flex justify-between text-sm"><span className="text-text-secondary">Envío</span><span className="text-[var(--success)]">Gratis</span></div>
        <Divider />
        <div className="flex justify-between"><span className="font-semibold">Total</span><span className="font-heading text-xl font-bold text-accent-primary">{formatPrice(subtotal)}</span></div>
      </div>
      ...
      <div className="bg-[var(--bg-surface)] rounded-xl border border-[var(--border)] p-4 flex flex-col gap-2">
        ...
        <p className="text-sm text-text-secondary">Calle Principal 123, Quito, Ecuador</p>
        <p className="text-sm text-text-secondary">Entrega estimada: 28-30 Abril, 2026</p>
      </div>
