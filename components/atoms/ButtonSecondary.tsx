...
export function ButtonSecondary({ label = 'Filtrar', icon = 'SlidersHorizontal', onClick, className = '', fullWidth }: ButtonSecondaryProps) {
  ...
  return (
    <button onClick={onClick}
      className={`flex items-center justify-center gap-2 h-10 px-5 rounded-lg bg-bg-elevated border border-[var(--border)] transition-colors hover:bg-[var(--bg-modal)] ${fullWidth ? 'w-full' : ''} ${className}`}>
