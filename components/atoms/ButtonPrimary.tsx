...
export function ButtonPrimary({ label = 'Agregar al carrito', icon = 'ShoppingCart', onClick, className = '', disabled, fullWidth, size = 'md' }: ButtonPrimaryProps) {
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      className={`flex items-center justify-center gap-2 ${h} px-5 rounded-lg bg-accent-primary hover:bg-[var(--accent-primary-hover)] disabled:opacity-50 transition-colors ${fullWidth ? 'w-full' : ''} ${className}`}>
